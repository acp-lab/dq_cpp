...
NMPCWrapper::NMPCWrapper() {
  ...
  new_time_steps = NULL;
  status = dq_quadrotor_acados_create_with_discretization(acados_ocp_capsule, N,
                                                          new_time_steps);
  ...
  acados_reference_end_state_.segment(0, kStateSize) =
      hover_state.template cast<double>();
}
...
void NMPCWrapper::setTrajectory(
    const Eigen::Ref<const Eigen::Matrix<double, kStateSize, kSamples>> states,
  ...
  acados_reference_end_state_.segment(0, kStateSize) =
      states.col(kSamples - 1).template cast<double>();
}

bool NMPCWrapper::prepare(
...
bool NMPCWrapper::update(
  ...
  for (int i = 0; i < N; i++) {
    dq_quadrotor_acados_update_params_sparse(acados_ocp_capsule, i, y_indeces,
                                             acados_in.yref + i * yRefSize,
                                             yRefSize);
  }
  dq_quadrotor_acados_update_params_sparse(acados_ocp_capsule, N, y_indeces,
                                           acados_in.yref_e, yRefSize);
  ...
  for (int ii = 0; ii <= nlp_dims->N; ii++)
    ocp_nlp_out_get(nlp_config, nlp_dims, nlp_out, ii, "x",
                    &acados_out.x_out[ii * NX]);
  for (int ii = 0; ii < nlp_dims->N; ii++)
    ocp_nlp_out_get(nlp_config, nlp_dims, nlp_out, ii, "u",
                    &acados_out.u_out[ii * NU]);

  return true;
}

void NMPCWrapper::getStates(
    Eigen::Matrix<double, kStateSize, kSamples> &return_states) {
  return_states = acados_states_.cast<double>();
}

void NMPCWrapper::getInputs(
    Eigen::Matrix<double, kInputSize, kSamples> &return_input) {
...
void NMPCWrapper::setWeightMatrices(std::vector<double> Q,
  ...
  for (int i = 0; i < N; i++) {
    dq_quadrotor_acados_update_params_sparse(
        acados_ocp_capsule, i, params_indeces, params.data(), params_size);
  }
}
