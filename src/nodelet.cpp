...
public:
  NMPCControlNodelet(const rclcpp::NodeOptions &options)
    ...
    //       is because we have thrusts in the control input implementation of
    //       NMPC!
    Eigen::Matrix4d mixer_matrix;
    if (platform_type_ == "voxl2" || platform_type_ == "raxl") {
      mixer_matrix << 1, 1, 1, 1, frame_dy_, frame_dy_, -frame_dy_, -frame_dy_,
          -frame_dx_, frame_dx_, frame_dx_, -frame_dx_, km_ / kf_, -km_ / kf_,
          km_ / kf_, -km_ / kf_;
    ...
    pub_trpy_cmd_ =
        this->create_publisher<quadrotor_msgs::msg::TRPYCommand>("trpy_cmd", 1);
    pub_ref_traj_ =
        this->create_publisher<nav_msgs::msg::Path>("reference_path", 1);
    pub_pred_traj_ =
        this->create_publisher<nav_msgs::msg::Path>("predicted_path", 1);
    pub_desired_pose_ =
        this->create_publisher<nav_msgs::msg::Odometry>("reference_pose", 1);
    force_tau_pub_ =
        this->create_publisher<geometry_msgs::msg::Wrench>("force_tau_cmd", 1);
    // pub_dual_ = this->create_publisher<mujoco_msgs::msg::Dual>("dual_cpp",
    // 10);
...
private:
  ...
  rclcpp::Publisher<geometry_msgs::msg::Wrench>::SharedPtr force_tau_pub_;
...
void NMPCControlNodelet::run() {
  ...
  // Publish solution
  publishControl(pred_state, pred_input, current_state);
  publishPrediction();
  publishReference();
}

void NMPCControlNodelet::odomCallback(
  ...
  Eigen::Vector4d current_odom_quat(
  ...
  controller_.setState(dual, odom_msg->header.stamp.sec +
...
void NMPCControlNodelet::imuCallback(
...
void NMPCControlNodelet::motorsCallback(
    const std_msgs::msg::Bool::SharedPtr motors_msg) {
  if (motors_msg->data)
...
void NMPCControlNodelet::publishControl(
    ...
    Eigen::Matrix<double, kStateSize, 1> current_state) {
  // Eigen::Matrix<double, kStateSize, 1> pred_state =
  // controller_.getPredictedState(); Eigen::Matrix<double, kInputSize, 1>
  // pred_input = controller_.getPredictedInput();
  quadrotor_msgs::msg::TRPYCommand trpy_msg;
  Eigen::Quaternion<double> orientation(pred_state(0), pred_state(1),
                                        pred_state(2), pred_state(3));
  ...
  torque(1) = pred_input(2);
  torque(2) = pred_input(3);

  Eigen::Vector3d Jw = inertia_matrix_ * omega;
  Eigen::Vector3d c = omega.cross(Jw);
  Eigen::Vector3d omega_dot = Eigen::Vector3d::Zero();

  omega_dot = inertia_matrix_.inverse() * (torque - c);

  linear_acceleration_angular_acceleration_msg.force.x = 0.0f;
  linear_acceleration_angular_acceleration_msg.force.y = 0.0f;
  linear_acceleration_angular_acceleration_msg.force.z = pred_input(0) / mass_;
  linear_acceleration_angular_acceleration_msg.torque.x = omega_dot(0);
  linear_acceleration_angular_acceleration_msg.torque.y = omega_dot(1);
  linear_acceleration_angular_acceleration_msg.torque.z = omega_dot(2);
  force_tau_pub_->publish(linear_acceleration_angular_acceleration_msg);
}
...
void NMPCControlNodelet::publishReference() {
  Eigen::Matrix<double, kStateSize, kSamples> reference_states =
      controller_.getReferenceStates();
  ...
  for (int i = 0; i < kSamples; i++) {
    ...
    H_plus_dual_part << dual_part(0), -dual_part(1), -dual_part(2),
        -dual_part(3), dual_part(1), dual_part(0), -dual_part(3), dual_part(2),
        dual_part(2), dual_part(3), dual_part(0), -dual_part(1), dual_part(3),
        -dual_part(2), dual_part(1), dual_part(0);
...
void NMPCControlNodelet::publishPrediction() {
  Eigen::Matrix<double, kStateSize, kSamples> predicted_states =
      controller_.getPredictedStates();
  nav_msgs::msg::Path path_msg;
  path_msg.header.stamp = clock_.now();
  path_msg.header.frame_id = frame_id_;
  path_msg.poses.reserve(kSamples);
